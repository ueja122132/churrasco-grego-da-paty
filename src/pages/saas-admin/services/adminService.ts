...
export const adminService = {
  ...
  async getMetrics(period: string = '30 dias'): Promise<SaaSMetrics> {
    ...
    const suspended = orgs.filter(o => o.status !== 'active').length;
    ...
    const newSignups = orgs.filter(o => new Date(o.created_at) >= startDate).length;
    ...
    const filteredPayments = allPayments.filter(p => new Date(p.paid_at) >= startDate);
    const mrr = filteredPayments.reduce((sum, p) => sum + Number(p.amount || 0), 0);
    const arr = mrr * (period === '12 meses' ? 1 : 12);

    // 2. Build History (Dynamic steps)
    ...
    if (period === '12 meses') {
      ...
      months.forEach((month, i) => {
        const rev = allPayments.filter(p => new Date(p.paid_at).getMonth() === i).reduce((sum, p) => sum + Number(p.amount || 0), 0);
        const gro = orgs.filter(o => new Date(o.created_at).getMonth() === i).length;
  ...
  async getCompanies() {
    ...
    const response = await fetch('/api/saas/companies-with-metrics', {
      headers: { 'Authorization': `Bearer ${session?.access_token}` }
    });

    if (!response.ok) throw new Error('Falha ao buscar empresas com métricas');
