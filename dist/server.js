...
if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceKey) {
    ...
    if (!supabaseUrl)
        console.error("\x1b[31m%s\x1b[0m", "ERRO: VITE_SUPABASE_URL está faltando!");
    if (!supabaseAnonKey)
        console.error("\x1b[31m%s\x1b[0m", "ERRO: VITE_SUPABASE_ANON_KEY está faltando!");
    if (!supabaseServiceKey)
        console.error("\x1b[31m%s\x1b[0m", "ERRO CRÍTICO: SUPABASE_SERVICE_ROLE_KEY está faltando!");
...
async function detectOrganization(host, slug, orgId) {
...
async function startServer() {
    ...
    const billingGuard = async (req, res, next) => {
        ...
        if (!orgId || (req.method === 'GET' && req.url.includes('/products')))
            return next();
        try {
            // Cache Check
    ...
    function processBilling(org, res, next) {
        ...
        try {
            ...
            if (metrics) {
                return res.json(orgs.map(org => ({
                    ...org,
                    metrics: metrics.find((m) => m.org_id === org.id) || { total_clients: 0, total_revenue: 0, monthly_orders: 0 }
                })));
            }
    ...
    app.get("/api/admin/global-metrics", superAdminGuard, async (req, res) => {
        try {
            ...
            const [{ count: totalOrgs }, { count: activeOrgs }, { data: revenueData }, { count: totalOrders }] = await Promise.all([
    ...
    app.get("/api/saas-payments/summary", superAdminGuard, async (req, res) => {
        try {
            ...
            const paidOrgIds = new Set(thisMonthPayments.map((p) => p.org_id));
            const totalEarnedAllTime = payments.reduce((acc, p) => acc + (p.amount || 0), 0);
            const totalThisMonth = thisMonthPayments.reduce((acc, p) => acc + (p.amount || 0), 0);
            const chargeableOrgs = orgs.filter((o) => !o.billing_exempt);
            const paidThisMonth = chargeableOrgs.filter((o) => paidOrgIds.has(o.id));
            const overdueOrgs = chargeableOrgs.filter((o) => {
                if (paidOrgIds.has(o.id))
                    return false;
                if (!o.billing_due_date)
                    return false;
                return new Date(o.billing_due_date) < now;
            });
            const exemptOrgs = orgs.filter((o) => o.billing_exempt);
            res.json({
                currentMonth,
                totalEarnedAllTime,
                totalThisMonth,
    ...
    app.post("/api/saas/subscribe/pix", async (req, res) => {
        ...
        if (existingOrg)
            return res.status(400).json({ error: "Este slug já está em uso. Escolha outro nome para a URL da sua loja." });
        ...
        if (!SAAS_MP_TOKEN) {
            ...
            return res.json({
                ...
                whatsapp: 'https://wa.me/5511999999999?text=' + encodeURIComponent(`Quero contratar o plano ${plan.name}.
Nome: ${name}
E-mail: ${email}
...
Slug: ${store_slug}`)
    ...
    app.get("/api/orders/:id/check-payment", async (req, res) => {
        try {
            ...
            if (!org?.mp_access_token)
                return res.json({ payment_status: order.payment_status });
    ...
    // PROTECTED
    app.get("/api/saas/clients", adminGuard, async (req, res) => {
        const profile = req.user;
        if (!profile)
            return res.status(401).json({ error: "Perfil não identificado" });
        try {
    ...
    app.get("/api/my-orders/:userId", async (req, res) => {
        ...
        if (orgIds.length > 0) {
            ...
            (allActive || []).forEach(ao => {
                if (!activeOrdersMap[ao.org_id])
                    activeOrdersMap[ao.org_id] = [];
    ...
    app.patch("/api/products/:id", async (req, res) => {
        ...
        const updatePayload = { name, description, price, ingredients, category, image_url };
        if (available !== undefined)
            updatePayload.available = available;
        if (promotional_price !== undefined)
            updatePayload.promotional_price = promotional_price;
    ...
    app.post("/api/:orgId/orders", checkSaaSLimits('orders'), async (req, res) => {
        try {
            ...
            if (org?.operating_hours) {
                ...
                const hours = org.operating_hours[dayName];
                if (hours) {
    ...
    app.patch("/api/orders/:id/status", async (req, res) => {
        ...
        if (courier_id)
            updatePayload.courier_id = courier_id;
        if (status === 'shipped')
...
- Nunca saia do personagem. Você É a Paty.`;
    ...
    app.post("/api/couriers", checkSaaSLimits('users'), async (req, res) => {
        ...
        const { data, error } = await supabase
            ...
            .maybeSingle();
        if (error)
            return res.status(400).json({ error: "Erro ao cadastrar: telefone já em uso ou dados inválidos." });
        res.json(data);
    });
    app.put("/api/couriers/:id", async (req, res) => {
        const { name, phone, email, password, commission_rate, org_id } = req.body;
        const updateData = { name, phone, email, commission_rate, org_id };
    ...
    const calculateOrderProfitAndCommission = (order, inventory, recipes, rate) => {
        let totalCost = 0;
    ...
    app.get("/api/courier/:id/stats", async (req, res) => {
        ...
        try {
            ...
            if (unpaidOrdersRes.error)
                throw unpaidOrdersRes.error;
    ...
    app.get('/api/courier/:courierId/location', (req, res) => {
        const loc = global.courierLocations?.[req.params.courierId];
        if (!loc)
            return res.status(404).json({ error: 'Entregador sem localização ativa' });
        res.json(loc);
    });
