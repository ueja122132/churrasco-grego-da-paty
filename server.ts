...
async function startServer() {
  ...
  const checkSaaSLimits = (type: 'orders' | 'products' | 'users') => {
    return async (req: any, res: any, next: any) => {
      ...
      try {
        ...
        if (!org || org.is_exempt) return next();
        ...
        if (limitValue === 'Ilimitado' || !limitValue) return next();
  ...
  app.patch("/api/organizations/:id/toggle-exemption", superAdminGuard, async (req, res) => {
    try {
      ...
      if (fetchError || !org) return res.status(404).json({ error: "Empresa não encontrada" });
  ...
  app.patch("/api/organizations/:id/cash-settings", async (req: any, res: any) => {
    const { id } = req.params;
  ...
  app.post("/api/organizations/:id/billing/pix", async (req, res) => {
    try {
      ...
      if (!org || !targetPlan) return res.status(404).json({ error: "Plano ou Loja não encontrada" });
      ...
      if (!mpAccessToken) return res.status(500).json({ error: "Serviço de pagamento não configurado no SaaS Master" });
      ...
      if (!mpRes.ok) throw new Error(mpResponse.message || "Erro no Mercado Pago");
  ...
  app.post("/api/saas/subscribe/pix", async (req, res) => {
    ...
    if (planError || !plan) return res.status(400).json({ error: "Plano inválido ou não encontrado." });
...
Slug: ${store_slug}`
  ...
  app.post("/api/organizations/:id/billing/pix", async (req, res) => {
    try {
      ...
      if (!plan_id) return res.status(400).json({ error: "Plano não selecionado" });
      ...
      if (orgError || !org) return res.status(404).json({ error: "Organização não encontrada" });

      // 2. Get all plans to compare prices
      const { data: plans } = await supabase.from('saas_plans').select('*');
      if (!plans) return res.status(500).json({ error: "Erro ao buscar planos" });

      const currentPlan = plans.find(p => p.slug === (org.plan || 'basic'));
      const newPlan = plans.find(p => p.slug === plan_id);

      if (!newPlan) return res.status(400).json({ error: "Plano selecionado inválido" });
      ...
      if (!SAAS_MP_TOKEN) return res.status(500).json({ error: "Serviço de pagamento SaaS não configurado." });
  ...
  app.patch("/api/orders/:id/status", async (req, res) => {
    ...
    if (targetRoom) {
      ...
      if (status === 'delivered') io.to(targetRoom).emit("order:payment_update", { id: parseInt(orderId), payment_status: 'paid' });
    }
  ...
  io.on("connection", (socket) => {
    ...
    socket.on('track:join', (data: { courierId: any }) => {
      ...
      if (last) socket.emit('courier:location:update', last);
    });

    socket.on('track:leave', (data: { courierId: any }) => {
      if (data.courierId) socket.leave(`track:${data.courierId}`);
    });

    // Courier sends their location
    socket.on('courier:location', (data: { courierId: any, courierName: string, latitude: number, longitude: number, timestamp: number }) => {
      if (!data.courierId) return;
    ...
    socket.on('courier:location:stop', (data: { courierId: any }) => {
      if (!data.courierId) return;
